...
def analyze_photo(image: np.ndarray) -> dict:
    """Face detection runs once and feeds both blur scoring and eyes-state
    detection. Blur only zooms into a subject region for a single clear
    subject (a portrait) — the face-crop threshold was calibrated for
    that case, and group photos aren't usually shot with shallow depth
    of field anyway, so those fall back to whole-frame scoring.
    """
